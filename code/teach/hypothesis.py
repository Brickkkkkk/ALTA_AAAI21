...
def cluster_hyps(X, Y, num_hyps, alpha, clf):
    ...
    for ii in range(np.minimum(num_classes-1,2)):
        class_combs.extend(itertools.combinations(list(range(num_classes)), ii + 1))
    ...
    all_combs = list(itertools.permutations(list(range(len(classifiers))), num_classes))
    subset_combs = random.sample(list(range(len(all_combs))), np.minimum(num_hyps,len(all_combs)))
...
def cluster_hyps_one_v_all(X, Y, alpha, clf):
    ...
    combs = list(itertools.combinations(list(range(num_classes)), 2))
    for cc in combs:
        inds = []
        for cur_class in cc:
...
def sparse_hyps(X, Y, num_hyps, one_v_all, clf, fit_gt=False):
    ...
    for hh in range(num_hyps):
        if one_v_all or (num_classes == 2):
            ...
            inds = random.sample(list(range(X.shape[1])), num_non_zero)
