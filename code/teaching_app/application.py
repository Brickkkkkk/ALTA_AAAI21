...
strat_files = glob.glob('data/explain.strat')
...
def initalize_session():
    ...
    session.clear()
    session['name'] = 'ddcc0817-14c2-4dcc-aa54-73e94b5cae08'
    session['response'] = []
    session['time'] = []
    session['gt_label'] = []
    session['history_urls'] = []
    session['history_labels'] = []
    session['strategy'] = random.choice(list(strats.keys()))
    # different users will see options in different order - order will be consistent for the length of session
    session['button_order'] = random.sample(list(range(len(class_names))), len(class_names))
...
def dashboard():
    ...
    if len(user_data) == 0:
        ...
        params['strat_names'] = list(strats.keys())
        params['test_scores'] = [0]*len(list(strats.keys()))
        params['users_per_strat'] = [0]*len(list(strats.keys()))
...
def teaching():
    ...
    if 'response' not in list(session.keys()):
        initalize_session()
    ...
    if request.method == 'POST':
        ...
        session['history_urls'].append('static/' + images[session['image_id'][session['current_id']]]['image_url'].replace(' ', '%20'))
        session['history_labels'].append(images[session['image_id'][session['current_id']]]['class_label'])
    ...
    params = {}
    params['image'] = 'static/' + images[image_id]['image_url']
    params['history_urls'] = session['history_urls']
    params['history_labels'] = session['history_labels']
    params['explain_image'] = 'static/' + images[image_id]['explain_url']
