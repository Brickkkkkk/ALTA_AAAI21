...
def save_teaching_sequence(teacher, alg_name, op_file_name):
    ...
    if 'rand' not in alg_name:
        if 'strict' in alg_name:
            results['im_ids'] = [int(i) for i in teacher.teaching_exs]
        ...
        if 'distance' in alg_name:
            results['im_ids'] = [int(i) for i in teacher.teaching_exs]
            results['display_explain_image'] = [0 for ii in range(num_train)]
        if 'actstu' in alg_name:
            results['im_ids'] = [int(i) for i in teacher.teaching_exs]
            results['im_seq'] = [int(i) for i in teacher.learning_seq]
            results['display_explain_image'] = [0 for ii in range(num_train)]
...
def save_teaching_images(dataset_train, dataset_test, op_file_name, url_root):
    ...
    for ii in range(len(dataset_train['im_files'])):
        im_data = {}
        im_data['image_url'] = url_root + str(dataset_train['im_files'][ii])#, encoding = "utf8")
        im_data['explain_url'] = url_root + str(dataset_train['explain_files'][ii])#, encoding = "utf8")
        im_data['class_label'] = int(dataset_train['Y'][ii])
        teaching_ims.append(im_data)

    # puts test images at the end
    for ii in range(len(dataset_test['im_files'])):
        im_data = {}
        im_data['image_url'] = url_root + str(dataset_test['im_files'][ii])#, encoding = "utf8")
        im_data['class_label'] = int(dataset_test['Y'][ii])
        teaching_ims.append(im_data)
...
def save_settings(dataset_train, dataset_test, experiment_id, num_random_test_ims, scale, op_file_name):
    ...
    settings['train_indices'] = list(range(len(dataset_train['im_files'])))
